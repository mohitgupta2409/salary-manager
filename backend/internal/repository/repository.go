...
// EmployeeRepository owns persistence for employees and the read-only
// salary-insight queries that aggregate over them.
//
// All List operations honour the embedded model.Pagination (offset/limit)
// on the request — a Limit <= 0 means "return all matching records".
type EmployeeRepository interface {
	...
	// Delete is a soft delete: the row is retained with is_active set to 0.
...
type CountryRepository interface {
	...
	List(ctx context.Context, req model.CountryListRequest) (*model.CountryListResult, error)
...
type DepartmentRepository interface {
	...
	List(ctx context.Context, req model.DepartmentListRequest) (*model.DepartmentListResult, error)
...
type JobTitleRepository interface {
	...
	List(ctx context.Context, req model.JobTitleListRequest) (*model.JobTitleListResult, error)
